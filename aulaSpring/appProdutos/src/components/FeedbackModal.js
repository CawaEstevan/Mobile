...
export default function FeedbackModal({ visivel, tipo, mensagem, onFechar }) {
  ...
  return (
    <Modal transparent={true} animationType="fade" visible={visivel} onRequestClose={onFechar}>
      <View style={styles.overlay}>
        <View style={styles.caixa}>
          <Ionicons name={ehSucesso ? 'checkmark-circle' : 'close-circle'} size={64} color={ehSucesso ? '#16a34a' : '#dc2626'} />
          <Text style={styles.mensagem}>{mensagem}</Text>
          <TouchableOpacity style={[styles.botao, ehSucesso ? styles.botaoSucesso : styles.botaoErro]} onPress={onFechar}>
...
const styles = StyleSheet.create({
  ...
  mensagem: { fontSize: 16, color: '#334155', textAlign: 'center', marginTop: 16, marginBottom: 24, lineHeight: 24 },
  botao: { paddingVertical: 12, paddingHorizontal: 40, borderRadius: 12 },
